...
function formatOkDebugDisplay(debug, stableGesture) {
    ...
    return [
        ...
        `angle roll=${formatOkDebugDegree(wake.rollDeg)} yaw=${formatOkDebugDegree(wake.yawDeg)} pitch=${formatOkDebugDegree(wake.pitchDeg)}/${formatOkDebugDegree(wake.pitchAbsMaxDeg)} face=${formatOkDebugDegree(wake.faceDeg)} src=${wake.orientationSource || '-'}`,
        `wakeOpen=${formatOkDebugBool(wake.eligible)} face=${formatOkDebugNumber(wake.faceOnScore)}/${formatOkDebugNumber(wake.faceOnMin)} world=${formatOkDebugNumber(wake.worldFaceOnScore)}/${formatOkDebugNumber(wake.worldFaceOnMin)} spread=${formatOkDebugNumber(wake.palmSpreadRatio)}/${formatOkDebugNumber(wake.palmSpreadMin)} plane=${formatOkDebugNumber(wake.fingerPlane)}/${formatOkDebugNumber(wake.fingerPlaneMax)}`,
        `fan=${formatOkDebugNumber(wake.fingerFan)} finger2d=${formatOkDebugNumber(wake.longFingerScreenLength)} pinky2d=${formatOkDebugNumber(wake.pinkyScreenLength)} pinkyOpen=${formatOkDebugBool(wake.pinkyOpen)}`,
...
function formatOkDebugLog(debug, stableGesture) {
    return `[SIGN_DEBUG] raw=${debug.rawGesture || '-'} stable=${stableGesture || '-'} ` +
        ...
        `roll=${formatOkDebugDegree(debug.wakeOpenDebug?.rollDeg)} yaw=${formatOkDebugDegree(debug.wakeOpenDebug?.yawDeg)} pitch=${formatOkDebugDegree(debug.wakeOpenDebug?.pitchDeg)}/${formatOkDebugDegree(debug.wakeOpenDebug?.pitchAbsMaxDeg)} faceDeg=${formatOkDebugDegree(debug.wakeOpenDebug?.faceDeg)} fail=${debug.failureReason || '-'}`;
}
