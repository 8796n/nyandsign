...
function formatOkDebugDisplay(debug, stableGesture) {
    ...
    return [
        ...
        `angle roll=${formatOkDebugDegree(wake.rollDeg)} yaw=${formatOkDebugDegree(wake.yawDeg)} pitch=${formatOkDebugDegree(wake.pitchDeg)} face=${formatOkDebugDegree(wake.faceDeg)} src=${wake.orientationSource || '-'}`,
...
function formatOkDebugLog(debug, stableGesture) {
    return `[SIGN_DEBUG] raw=${debug.rawGesture || '-'} stable=${stableGesture || '-'} ` +
        ...
        `roll=${formatOkDebugDegree(debug.wakeOpenDebug?.rollDeg)} yaw=${formatOkDebugDegree(debug.wakeOpenDebug?.yawDeg)} pitch=${formatOkDebugDegree(debug.wakeOpenDebug?.pitchDeg)} faceDeg=${formatOkDebugDegree(debug.wakeOpenDebug?.faceDeg)} fail=${debug.failureReason || '-'}`;
}
