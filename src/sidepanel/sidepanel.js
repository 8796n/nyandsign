...
function formatOkDebugDegree(value) {
...
function formatOkDebugDisplay(debug, stableGesture) {
    ...
    return [
        ...
        `indexExt=${formatOkDebugBool(debug.indexExtended)} indexPoint=${formatOkDebugBool(debug.indexPointLike)} pointDir=${debug.pointDirection || '-'} indexOk=${formatOkDebugBool(debug.indexExtendedOkPinch)} indexCurl=${formatOkDebugBool(debug.indexCurled)} indexBent=${formatOkDebugBool(debug.indexBentForOk)} indexStr=${formatOkDebugNumber(debug.indexStraightness)} otherExt=${debug.otherExtendedCount ?? '-'}`,
        ...
        `wakeOpen=${formatOkDebugBool(wake.eligible)} pose roll=${formatOkDebugDegree(wake.rollDeg)} yaw=${formatOkDebugDegree(wake.yawDeg)}/${formatOkDebugDegree(wake.yawAbsMaxDeg)} pitch=${formatOkDebugDegree(wake.pitchDeg)}/${formatOkDebugDegree(wake.pitchAbsMaxDeg)} face=${formatOkDebugDegree(wake.faceDeg)}/${formatOkDebugDegree(wake.faceMaxDeg)} src=${wake.orientationSource || '-'}`,
        `shape fan=${formatOkDebugNumber(wake.fingerFan)}/${formatOkDebugNumber(WAKE_OPEN_FINGER_FAN_MIN)} plane=${formatOkDebugNumber(wake.fingerPlane)}/${formatOkDebugNumber(wake.fingerPlaneMax)} fingers=${formatOkDebugBool(wake.longFingersExtended)} straight=${formatOkDebugBool(wake.fingersStraight)} pinky=${formatOkDebugBool(wake.pinkyOpen)} thumb=${formatOkDebugBool(wake.thumbOpen)} palmOpen=${formatOkDebugBool(wake.palmOpen)}`,
        `legacy faceScore=${formatOkDebugNumber(wake.faceOnScore)} worldScore=${formatOkDebugNumber(wake.worldFaceOnScore)} spread=${formatOkDebugNumber(wake.palmSpreadRatio)}`,
...
function formatOkDebugLog(debug, stableGesture) {
    return `[SIGN_DEBUG] raw=${debug.rawGesture || '-'} stable=${stableGesture || '-'} ` +
        ...
        `indexExt=${formatOkDebugBool(debug.indexExtended)} indexPoint=${formatOkDebugBool(debug.indexPointLike)} pointDir=${debug.pointDirection || '-'} indexOk=${formatOkDebugBool(debug.indexExtendedOkPinch)} indexBent=${formatOkDebugBool(debug.indexBentForOk)} indexStr=${formatOkDebugNumber(debug.indexStraightness)} otherExt=${debug.otherExtendedCount ?? '-'} ` +
        `thumbFold=${formatOkDebugBool(debug.thumbFolded)} thumbAway=${formatOkDebugBool(debug.thumbExtendedAway)} thumbPalm=${formatOkDebugNumber(debug.thumbPalmDist)} fourThumb=${formatOkDebugBool(debug.fourThumbTucked)} palm=${debug.palmFacing ? 'front' : 'back'} ` +
        `roll=${formatOkDebugDegree(debug.wakeOpenDebug?.rollDeg)} yaw=${formatOkDebugDegree(debug.wakeOpenDebug?.yawDeg)}/${formatOkDebugDegree(debug.wakeOpenDebug?.yawAbsMaxDeg)} pitch=${formatOkDebugDegree(debug.wakeOpenDebug?.pitchDeg)}/${formatOkDebugDegree(debug.wakeOpenDebug?.pitchAbsMaxDeg)} faceDeg=${formatOkDebugDegree(debug.wakeOpenDebug?.faceDeg)}/${formatOkDebugDegree(debug.wakeOpenDebug?.faceMaxDeg)} fail=${debug.failureReason || '-'}`;
}
...
function resetCameraPreviewFilter() {
    el.cameraVideo?.style.removeProperty('--camera-preview-filter');
}
...
function syncCameraPreviewFilter(track = null) {
    const profile = CameraRuntime.xrealCameraProfile(track);
    if (profile === CameraRuntime.XREAL_CAMERA_PROFILE_MONO) {
        el.cameraVideo?.style.setProperty('--camera-preview-filter', 'brightness(1.35) contrast(1.08)');
        return;
    }
    resetCameraPreviewFilter();
}
