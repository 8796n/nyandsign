...
メディア操作モードでは YouTube、Spotify、Netflix などの再生を操作できます。ブラウザ操作モードではページスクロール、戻る/進む、タブ切り替え、ズーム、カーソル移動などを操作できます。実験的機能として、ページ内の仮想カーソルをハンドサインで動かすポインタ操作モードも用意しています。
...
設定で「ポインタ操作モード」を ON にした場合だけ表示される実験的な操作モードです。ページ内に仮想カーソルを表示し、ハンドサインを保持したまま手を動かしてカーソルを移動できます。
...
NyandSign には「メディア操作」と「ブラウザ操作」の操作モードがあります。実験的なポインタ操作モードを ON にすると、「ポインタ操作」も切り替え候補に追加されます。サイドパネル上部のモード切り替えボタン、または両手サインで切り替えられます。
...
両手サインは一度発火すると、サインを解除するまで同じ機能を繰り返し発火しません。ポインタ操作モードが OFF の場合、モード切り替えはメディア操作とブラウザ操作だけを循環します。
