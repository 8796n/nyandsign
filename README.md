...
- ウェイク用パー判定を 3DoF 相当の手の姿勢（face / yaw / pitch / roll）中心に整理し、手がカメラ正面を向いているかをより直感的に判定
- キーボード操作中の手や斜め・寝た手がウェイクサインとして誤発火しにくいよう、ウェイク用パーの向き・奥行き・指の開き判定を調整
- 掌側と手の甲側のランドマーク特性に合わせ、手の甲側のウェイク用パー判定を個別に調整
- パーを出したまま待機時間を超えた場合に再ウェイクしないよう、一度手を離すまで再認識を抑制
- ウェイクサインとして不足している要素（手を正面へ、上下傾きを抑える、指を少し開く等）を表示し、調整しやすく改善
- 両手パーサインを追加し、両手サインの割り当て候補を拡張
- ポインタ移動の開始・復帰を上指差しだけでなく左右指差しにも広げ、同じ移動アクション間の継続を改善
- 指差しが角度によってサムズアップに誤判定されるケースを抑制
- UVC1 の推論入力を原寸に保ち、低解像度白黒カメラでの認識安定性を改善
